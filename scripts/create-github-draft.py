...
def release_for_tag(repository: str, tag: str) -> dict | None:
    return next(
        (release for release in github_releases(repository) if release["tag_name"] == tag),
        None,
    )
...
def main() -> None:
    ...
    if release is not None and not release["draft"]:
        raise RuntimeError(f"Published GitHub release {tag} already exists")
    if release is None and (
        subprocess.run(
    ...
    if release is None:
        ...
        release = release_for_tag(repository, tag)
        if release is None:
            raise RuntimeError(f"GitHub did not return the new draft {tag}")
    ...
    try:
        ...
        raise RuntimeError(
            f"GitHub draft {tag} was kept so its uploaded files can be reused"
        ) from error
