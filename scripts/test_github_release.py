...
class GitHubReleaseDraftTests(unittest.TestCase):
    ...
    def test_finds_a_draft_in_the_release_list(self, check_output):
        check_output.return_value = (
            '[[{"tag_name":"v1.0.4","draft":true}],'
            '[{"tag_name":"v1.0.3","draft":false}]]'
        )

        release = MODULE.release_for_tag("owner/repository", "v1.0.4")
    ...
    @mock.patch.object(MODULE.subprocess, "check_output", return_value="[[]]")
    def test_returns_none_for_an_unused_tag(self, _check_output):
        self.assertIsNone(MODULE.release_for_tag("owner/repository", "v1.0.4"))
